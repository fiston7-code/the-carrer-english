  ...
  return (
    <main className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 pt-6 pb-4">
        ...
        <span className="text-sm text-steel">Back</span>
      </div>
      ...
      <div className="flex-1 px-4 py-4 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          {isLive && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-orange-400 text-xs font-semibold uppercase">Live</span>
            </div>
          )}
          {room.category && (
          ...
          {room.industry && (
            <span className="text-xs bg-muted text-steel px-3 py-1 rounded-full">
              {room.industry}
            </span>
          )}
        </div>
        ...
        <div>
          ...
          {room.description && (
            <p className="text-steel mt-2 text-sm leading-relaxed">{room.description}</p>
          )}
        </div>

        {/* Coach card */}
        {coach && (
          <div className="bg-surface rounded-2xl p-4 flex items-center gap-4 border border-border">
            <div className="w-14 h-14 rounded-full overflow-hidden bg-muted shrink-0">
              {coach.avatar_url ? (
                ...
                <div className="w-full h-full bg-gold/20 text-gold flex items-center justify-center text-xl font-bold">
                  {coach.full_name?.[0]}
                </div>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-semibold truncate text-foreground">{coach.full_name}</span>
                {coach.is_verified && (
      ...
      <div className="px-4 pb-10 flex flex-col gap-3">
        {isCoach ? (
          ...
          <>
            ...
            >
              <Mic size={18} />
              {isJoining ? "Connecting..." : "Join as Speaker"}
            </button>
            <button
            ...
            >
              <Headphones size={18} />
              {isJoining ? "Connecting..." : "Join as Listener"}
            </button>
          </>
        )}
      </div>
