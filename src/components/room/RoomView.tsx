...
type DbParticipant = {
  ...
  role: string                // 'coach' | 'speaker' | 'listener'
  is_muted: boolean
  hand_raised: boolean
...
function RoomInner({
  ...
  const coaches   = dbParticipants.filter((p) => p.role === 'coach')
  const speakers  = dbParticipants.filter((p) => p.role === 'speaker')
...
export default function RoomView({ room, profile, userId, isCoach }: Props) {
  ...
  useEffect(() => {
    // Upsert : enregistre le participant à l'entrée
  ...
  // Fetch feedbacks + realtime
  // Student : seulement les siens (student_id = userId)
  // Coach   : tous les feedbacks de la room
  useEffect(() => {
    let query = supabase
      .from('live_feedbacks')
    ...
    query.then(({ data }) => setFeedbacks((data ?? []) as Feedback[]))
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // N'ajoute que si destiné à ce user (ou si coach qui voit tout)
        if (isCoach || f.student_id === userId) {
