...
// Calcule un score sur 100 basé sur les données disponibles
function computeScore(totalMinutes: number, sessions: number, corrections: number) {
  const timeScore   = Math.min(totalMinutes / 600, 1) * 40   // 40pts max — 600min cible
  const sessionScore = Math.min(sessions / 20, 1) * 40        // 40pts max — 20 sessions cible
  const corrScore    = Math.min(corrections / 50, 1) * 20     // 20pts max — 50 corrections reçues
...
function getWeeklyData(sessions: any[]) {
  ...
  return Array.from({ length: 8 }, (_, i) => {
    // Semaine i en remontant depuis aujourd'hui
...
export default async function ProgressPage() {
  ...
  // Feedbacks reçus par ce student
  const { data: feedbacks } = await supabase
    .from("live_feedbacks")
    .select("id, type, mistake, correction, explanation, created_at, coach_id")
  ...
  return (
    <main className="min-h-screen bg-background text-foreground max-w-md mx-auto px-4 py-6 flex flex-col gap-5 pb-12">
      ...
      <div className="flex items-center gap-3">
        <Link href="/dashboard" className="w-9 h-9 rounded-xl bg-surface border border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft size={16} />
        </Link>
      ...
      <div className="bg-surface border border-border rounded-2xl p-4 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full overflow-hidden shrink-0 ring-2 ring-gold/30">
          {profile.avatar_url ? (
            <Image src={profile.avatar_url} alt={profile.full_name ?? ''} width={64} height={64} className="object-cover w-full h-full" />
          ) : (
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative w-24 h-24 shrink-0">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle cx="50" cy="50" r="45" fill="none" stroke="hsl(43 96% 56%)" strokeWidth="8"
          ...
          <div>
            ...
            <span className="text-xs bg-gold/20 text-gold px-2.5 py-0.5 rounded-full font-medium mt-2 inline-block">
              Level {getLevelLabel(profile.english_level)} → Next level
            </span>
        ...
        <div className="flex items-end gap-2 h-28">
          {weeklyData.map(({ label, value }) => (
            <div key={label} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full bg-gold rounded-t-md transition-all" style={{ height: `${(value / maxWeekly) * 100}%`, minHeight: value > 0 ? 4 : 0 }} />
